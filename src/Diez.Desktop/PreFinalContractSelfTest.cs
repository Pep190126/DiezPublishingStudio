...
internal static class PreFinalContractSelfTest
{
    public static void Run()
    {
        ...
        if (!ProductInfo.WindowTitle.Contains("1.0 RC8", StringComparison.Ordinal) ||
            !ProductInfo.WindowTitle.Contains("SW-FLOW-4", StringComparison.Ordinal))
            throw new InvalidOperationException("Il titolo della build non espone correttamente RC8 / SW-FLOW-4.");
        ...
        if (!ProductInfo.Subtitle.Contains("Single-window", StringComparison.OrdinalIgnoreCase) ||
            !ProductInfo.Subtitle.Contains("SW-FLOW-4", StringComparison.OrdinalIgnoreCase))
