...
internal static class ImageCollectionWorkspaceTabsUi
{
    public static void Attach(MainWindow window)
    {
        ...
        {
            if (host is null || bookTabs is null || !host.IsAttachedToVisualTree())
            {
    ...
    private static bool IsAttachedToVisualTree(this Control control) => control.VisualRoot is not null;
}
...
internal sealed class ImageCollectionTabWorkspace
{
    ...
    private readonly Image _preview = new() { Stretch = Avalonia.Media.Stretch.Uniform, Height = 390 };
    private readonly TextBlock _selectedInfo = new() { Text = "Seleziona un'immagine nel tab Database.", FontSize = 18, TextWrapping = Avalonia.Media.TextWrapping.Wrap };
    private readonly TextBox _title = new();
    private readonly TextBox _request = new() { AcceptsReturn = true, Height = 105, TextWrapping = Avalonia.Media.TextWrapping.Wrap };
    private readonly TextBox _description = new() { AcceptsReturn = true, Height = 260, TextWrapping = Avalonia.Media.TextWrapping.Wrap };
    private readonly TextBox _rules = new() { AcceptsReturn = true, Height = 190, TextWrapping = Avalonia.Media.TextWrapping.Wrap };
    ...
    private readonly TextBox _aiInstructions = new() { AcceptsReturn = true, Height = 270, IsReadOnly = true, TextWrapping = Avalonia.Media.TextWrapping.Wrap };
    ...
    {
        _window = window;
        Contents = [BuildDatabase(), BuildBookType(), BuildChecks(), BuildAi(), BuildExport()];
    ...
    public async Task RefreshAsync()
    {
        ...
        var selectedCode = _selected?.Code;
        _jobs = ImageCollectionWorkspaceService.Jobs(project);
        RefreshList(selectedCode);
        _rules.Text = ImageCollectionWorkspaceService.GetConsistencyRules(project);
    ...
    private Control BuildDatabase()
    {
        ...
        return new Grid
        {
            ...
            RowSpacing = 8,
            Children = { top, series.WithGridRow(1), _summary.WithGridRow(2), _list.WithGridRow(3) }
        };
    }

    private Control BuildBookType()
    {
        ...
        approve.Click += async (_, _) => await SetStatusAsync(AiProductionService.StatusApproved);
        redo.Click += async (_, _) => await SetStatusAsync(AiProductionService.StatusNeedsRevision);
        ...
        var previewPanel = new StackPanel
        {
            ...
            Children =
            {
                ...
                new TextBlock
                {
                    Text = "L'originale resta nel .diez. Sostituisci modifica solo questa posizione IMG-### e non sposta le altre immagini.",
        ...
        var editor = new ScrollViewer
        {
            ...
            Content = new StackPanel
            {
                ...
                {
                    Field("Titolo", _title),
                    Field("Cosa deve rappresentare / richiesta", _request),
        ...
        return new Grid
        {
            ...
            ColumnSpacing = 12,
            Children = { previewPanel, editor.WithGridColumn(1) }
        };
    }
            ...
            Children =
            {
                ...
                new StackPanel
                {
                    ...
                    Children =
                    {
                        ...
                        _rules,
                        new StackPanel { Orientation = Orientation.Horizontal, Spacing = 8, Children = { saveRules, check, prepare } }
                    }
                }.WithGridRow(1),
                ...
                new TextBlock
                {
                    Text = "Diez segnala le posizioni reali (IMG-###). Preparare le segnalate non cambia nessuna immagine: le marca soltanto per la correzione/rigenerazione.",
                    TextWrapping = Avalonia.Media.TextWrapping.Wrap
                }.WithGridRow(4)
            }
        };
        ...
        return new ScrollViewer
        {
            ...
            Content = new StackPanel
            {
                ...
                Children =
                {
                    ...
                    new TextBlock
                    {
                        Text = "La correzione resta legata allo stesso IMG-###. L'AI prepara una nuova versione; sei tu a controllarla e approvarla.",
                        TextWrapping = Avalonia.Media.TextWrapping.Wrap
                    },
    ...
    private Control BuildExport()
    {
        ...
        return new StackPanel
        {
            ...
            Children =
            {
                ...
                new TextBlock
                {
                    Text = "Documento modificabile (DOCX) è un formato di scambio, non una destinazione legata a Word. Diez mantiene aperto lo stesso punto di consegna per flussi con LibreOffice/OpenOffice e, in futuro, Google Documenti.",
                    TextWrapping = Avalonia.Media.TextWrapping.Wrap
                }
            }
        };
    ...
    private async Task CreateSeriesAsync()
    {
        ...
        var theme = (_newTheme.Text ?? string.Empty).Trim();
        AiImageBatchService.CreateImageSeries(project, count, theme, "Tavola");
    ...
    private void RefreshList(string? keepCode = null)
    {
        ...
        {
            var matchesText = query.Length == 0 ||
                              j.Code.Contains(query, StringComparison.OrdinalIgnoreCase) ||
                              j.Title.Contains(query, StringComparison.OrdinalIgnoreCase) ||
                              ImageCollectionDescriptionService.GetDescription(j).Contains(query, StringComparison.OrdinalIgnoreCase);
        ...
        try
        {
            ...
            var index = !string.IsNullOrWhiteSpace(keepCode)
                ? _displayed.FindIndex(j => string.Equals(j.Code, keepCode, StringComparison.OrdinalIgnoreCase))
                : (_displayed.Count > 0 ? 0 : -1);
    ...
    private async Task LoadSelectedAsync(bool force = true)
    {
        ...
        if (_selected.ResultMaterialId.HasValue)
        {
    ...
    {
        _selected = null;
        _selectedInfo.Text = "Seleziona un'immagine nel tab Database.";
    ...
    private async Task ReplaceImageAsync()
    {
        ...
        var result = await AiProductionService.AttachResultFileAsync(project, path, _selected, selected.Path.LocalPath);
    ...
    {
        if (_selected is null || !TrySession(out var project, out var path)) return;
        var result = string.Equals(status, AiProductionService.StatusApproved, StringComparison.Ordinal)
            ? AiProductionService.Approve(project, _selected)
            : AiProductionService.NeedsRevision(_selected);
    ...
    private async Task RunChecksAsync()
    {
        ...
        _checkSummary.Text = _lastChecks.Count == 0
            ? "Nessun problema misurabile trovato nella raccolta."
            : $"{_lastChecks.Count} segnalazioni · {actionable} immagini richiedono un intervento concreto.";
    }
    ...
    private async Task PrepareFlaggedAsync()
    {
        ...
        var codes = _lastChecks.Where(c => c.NeedsAction && c.Code.StartsWith("IMG-", StringComparison.OrdinalIgnoreCase))
            .Select(c => c.Code).Distinct(StringComparer.OrdinalIgnoreCase).ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var job in ImageCollectionWorkspaceService.Jobs(project).Where(j => codes.Contains(j.Code)))
            AiProductionService.NeedsRevision(job);
        if (codes.Count > 0) await ProjectFileStore.SaveAsync(path, project);
        SetStatus(codes.Count == 0
            ? "Non ci sono immagini da preparare per la correzione."
            : $"{codes.Count} immagini segnate da rifare. Nessun file è stato modificato: ora puoi creare il pacchetto AI soltanto per queste posizioni.");
        await RefreshAsync();
    }

    private void PrepareSelectedInstructions()
    {
        if (_selected is null || !TrySession(out var project, out _))
        {
            _aiInstructions.Text = "Seleziona prima un'immagine nel tab Database.";
            return;
        }
    ...
    private async Task ExportCorrectionPackAsync()
    {
        ...
        var result = await AiImageBatchService.ExportPackXlsxAsync(
            project,
            file.Path.LocalPath,
            _provider.SelectedItem?.ToString() ?? AiImageBatchService.ProviderOpenAi,
            _advancedModel.IsChecked == true,
            onlyMissingOrToRedo: true);
        SetStatus(result.Message);
    }
    ...
    private void UpdateExportInfo()
    {
        ...
        var externalPart = !string.Equals(mode, ImageCollectionLayoutExportService.Internal, StringComparison.Ordinal);
        _includeDescriptions.IsVisible = externalPart;
        _descriptionFormat.IsVisible = externalPart && _includeDescriptions.IsChecked == true;
        _exportInfo.Text = mode switch
        {
            ImageCollectionLayoutExportService.Internal => "Documento modificabile (DOCX) con le immagini già inserite. Gli originali restano comunque conservati nel progetto.",
            ImageCollectionLayoutExportService.Both => "Un unico pacchetto con documento modificabile + immagini originali separate. Le descrizioni restano facoltative.",
            _ => "Immagini originali separate per impaginare fuori Diez. Le descrizioni sono facoltative e, se richieste, possono essere TXT o DOCX con lo stesso nome base dell'immagine."
        };
    }
