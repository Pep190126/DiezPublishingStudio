...
internal static class SingleWindowV5UiContractProbe
{
    ...
    {
        var temp = Path.Combine(Path.GetTempPath(), "diez-ui-v5-" + Guid.NewGuid().ToString("N") + ".diez");
        try
        {
            // 1. No project: the first visible logical page must be the start page.
            ...
            var project = ProjectFileStore.Create("Coloring V5 Contract");
            ...
            // 3. Coloring: quantity page, no legacy radios.
            ...
            var quantityEditors = Descendants(pageHost.Content as Control).OfType<TextBox>()
                .Where(t => t.IsEnabled && !t.IsReadOnly).ToList();
            ...
            // 4. Prompt page: exactly the three human-facing editors must exist and support undo/redo.
