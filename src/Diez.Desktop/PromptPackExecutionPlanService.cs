...
4. Use exactly ONE image-generation attempt in that clean-room chat. Inspect the actual output against `PRIMARY SUBJECT — HARD LOCK`, `STYLE — HARD LOCK`, the independent Coloring HARD states, line weight and `COMPOSITION — HARD LOCK`. If any HARD lock fails, return that task as FAILED without an asset; do not contaminate the same chat with repeated edit/retry cycles.
