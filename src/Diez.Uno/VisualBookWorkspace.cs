...
internal static class VisualBookWorkspace
{
        ...
        Action showAiCenter)
    {
        ...
        var root = PageRoot(
            ...
            "Quantità, profilo, Prompt atomici, job AI, Vision e finalizzazione usano lo stesso stato canonico del Core. L'interfaccia è ancora Preview: la rifiniremo dopo la prima installazione.");

        if (!BookTypeCatalog.IsVisual(type))
        {
            root.Children.Add(Card("Tipo libro non visuale", new TextBlock
            {
                Text = "Questo workspace è disponibile per Coloring Book, Raccolta immagini e Libro illustrato.",
        ...
        var subject = Editor(FirstNonBlank(setup.Subject, document.GetUiString("Visual.Subject")), "Soggetto/i principali.", 95);
        var environment = Editor(FirstNonBlank(setup.Environment, document.GetUiString("Visual.Environment")), "Ambientazione / scenario.", 95);
        ...
        root.Children.Add(Card("1 · Piano del libro", Vertical(
        ...
        else
        {
            ...
            root.Children.Add(Card("Profilo immagini", Vertical(
                ...
                Labeled("Trattamento linee", line), Labeled("Stile resa", rendering), Labeled("Sfondo", background),
                Labeled("Punto di vista", viewpoint), readable, noText, clarity, sameScale, Labeled("Note", notes))));
        }

        var mustDo = Editor(document.GetUiString("Prompt.MustDo"), "Cosa deve esserci / cosa deve fare l'immagine.", 90);
        var mustNot = Editor(document.GetUiString("Prompt.MustNot"), "Cosa deve essere escluso.", 80);
        ...
        async Task<bool> SaveSetupAsync()
        {
            ...
            if (coloringProfile is not null)
            {
                document.SaveColoringSetup(
                    parsed, subject.Text, environment.Text, consistent.IsChecked == true,
            ...
            else if (imageProfile is not null)
            {
                document.SaveImageBookSetup(
                    type, parsed, subject.Text, environment.Text, consistent.IsChecked == true,
                    consistent.IsChecked == true ? consistencyRules.Text : string.Empty,
                    imageProfile());
            }

            // These keys are kept only as migration drafts for older Uno builds; Core is authoritative.
            ...
            document.SetUiString("Prompt.MustNot", mustNot.Text);
        ...
        root.Children.Add(Card("3 · Prompt Pack atomico", Vertical(
            ...
            Horizontal(
                ...
                {
                    if (currentPack is null) await BuildPackAsync();
                    if (currentPack is null) return;
                    var result = document.EnsureVisualReadyJobs(currentPack);
                    await save();
        ...
        var checks = string.Join(Environment.NewLine, publication.Checks
        ...
        root.Children.Add(Card("Stato del libro", Vertical(
            ...
            new TextBlock { Text = checks, TextWrapping = TextWrapping.Wrap })));

        root.Children.Add(Card("4 · Freeze, Publication Candidate ed export finale", Vertical(
            new TextBlock
            {
                Text = "Queste azioni diventano disponibili solo quando le immagini finali sono state importate, approvate in Vision, portate nel libro e incorporate nel .diez.",
                TextWrapping = TextWrapping.Wrap
            },
            Horizontal(
            ...
            Horizontal(
                AsyncButton("Esporta immagini finali ZIP", async () =>
                {
                    ...
                    catch (Exception ex) { report("Errore export immagini: " + ex.GetBaseException().Message); }
                }),
                AsyncButton("Esporta pacchetto pubblicazione", async () =>
                {
                    ...
                    catch (Exception ex) { report("Errore export pubblicazione: " + ex.GetBaseException().Message); }
                }))));
