...
# 1) Core readiness: theme state must reflect both canonical frozen subjects and compatible legacy concrete subjects.
bridge = 'src/Diez.Core/DiezVisualThemeFrontendBridge.cs'
old = '''        var active = MultiSubjectProfileService.ActiveSubjects(MultiSubjectProfileService.Load(project));
...
'''
new = '''        var multi = MultiSubjectProfileService.Load(project);
...
# 2) Uno: do not navigate from Definition to Prompt when the subject plan is unresolved.
workspace = 'src/Diez.Uno/VisualBookWorkspace.cs'
old = '''            AsyncButton("Salva e continua → Prompt", async () =>
            {
...
'''
new = '''            AsyncButton("Salva e continua → Prompt", async () =>
            {
                if (!await SaveSetupAsync()) return;
...
# 3) Uno: second defensive gate if Phase 2 is reached from history/navigation with unresolved state.
old = '''        var promptPreview = Editor(string.Empty, "Prompt compilato", 420);
        promptPreview.IsReadOnly = true;

        void CompilePreview()
'''
new = '''        var promptPreview = Editor(string.Empty, "Prompt compilato", 420);
        promptPreview.IsReadOnly = true;
...
# 4) Regression: reproduce the installed UI round-trip after accepting a theme proposal.
regression = Path('tests/Diez.VisualSemanticRegression/Round561PromptGateRegression.cs')
regression.write_text(r'''using System.Runtime.CompilerServices;
